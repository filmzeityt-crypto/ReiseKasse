/* Reisekasse — rendering layer. Plain DOM string templates + delegated
   events. Inputs inside modals are intentionally uncontrolled (read from
   the DOM on save) so re-rendering never fights the user's typing. */
...
(function () {
  ...
  /* ---------------- icons (simple inline SVG, no dependency) ---------------- */
  ...
  function rerenderRateStatus() {
    var el = document.getElementById("rate-status-text");
    if (el) el.textContent = R.rateStatusText();
    var sub = document.getElementById("header-sub");
    if (sub) sub.textContent = R.getState().trip.baseCurrency + " · " + R.rateStatusText();
  }
  ...
  function renderHeader(s) {
    return '<div class="app-header">' +
      ...
      '<div class="sub" id="header-sub">' + escapeHtml(s.trip.baseCurrency) + " · " + R.rateStatusText() + (R.saveErrorFlag() ? " · Speichern fehlgeschlagen" : "") + "</div>" +
      "</div></div>" +
  ...
  function renderOverview(s) {
    var d = R.computeDerived();
    var html = '<div class="stat-row">' +
      statStub("Gesamt", R.fmtMoney(d.totalBase, d.base), "wallet") +
  ...
  function renderList(s) {
    var expenses = ui.filterCat === "all" ? s.expenses : s.expenses.filter(function (e) { return e.category === ui.filterCat; });
    var pills = '<div class="pill-row">' +
    ...
    if (expenses.length === 0) {
      return pills + '<div class="stub empty-hint">Keine Ausgaben in dieser Kategorie.</div>';
    }
    ...
    var rows = expenses.map(function (e) {
      ...
      return '<div class="stub expense-row">' +
        ...
        '<div class="expense-meta">' + escapeHtml(e.date) + (e.note ? " · " + escapeHtml(e.note) : "") + "</div></div>" +
        '<div class="expense-amount"><div class="main mono">' + R.fmtMoney(e.amount, e.currency) + "</div>" + sub + "</div>" +
        '<button class="del-btn" data-action="delete" data-id="' + e.id + '" aria-label="Löschen">' + icon("trash", 16) + "</button>" +
        "</div>";
    }).join("");
  ...
  function renderSettingsModal(s) {
    ...
    return modalShell("Einstellungen",
      ...
      "Bereits erfasste Ausgaben behalten den Kurs, der beim Speichern galt — sie ändern sich nachträglich nie. " +
      "Manche exotischeren Währungen liefert die automatische Prüfung nicht; die kannst du hier weiterhin von Hand pflegen.</div>" +
      "</div>" +
  ...
  function bindEvents(s) {
    appEl.querySelectorAll("[data-action]").forEach(function (el) {
      el.addEventListener("click", function (evt) {
        ...
        if (action === "backdrop" && evt.target === el) { ui.modal = null; render(); return; }
        if (action === "stop") { evt.stopPropagation(); return; }
        ...
        if (action === "save-expense") {
          ...
          var date = document.getElementById("f-date").value || R.todayISO();
          var note = document.getElementById("f-note").value.trim();
          R.addExpense({ amount: amount, currency: currency, category: addFormCat, date: date, note: note });
          ui.modal = null;
          render();
          return;
        }
        ...
        if (action === "add-currency") {
          ...
          if (!code) return;
          var st = R.getState();
          if (st.rates[code]) { codeInput.value = ""; return; }
          st.rates[code] = 1;
        ...
        if (action === "refresh-rates") {
          ...
          R.refreshRatesIfNeeded(true).then(function () {
            var st2 = R.getState();
            document.getElementById("rate-status-text").textContent = R.rateStatusText();
            document.getElementById("rate-list").innerHTML = rateRows(st2);
            btn.disabled = false;
        ...
        if (action === "save-settings") {
          ...
          var rateInputs = document.querySelectorAll("#rate-list input[data-code]");
          var newRates = {};
          rateInputs.forEach(function (inp) {
            var code = inp.getAttribute("data-code");
            var v = parseFloat(inp.value);
            newRates[code] = isNaN(v) || v <= 0 ? 1 : v;
          });
  ...
  window.ReisekasseUI = { init: render, rerenderRateStatus: rerenderRateStatus };
})();
