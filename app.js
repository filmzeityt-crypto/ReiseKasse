...
(function () {
  ...
  window.Reisekasse = {
    ...
    exportCsv: function () {
      ...
      var base = state.trip.baseCurrency;
      var rows = [["Datum", "Art", "Kategorie", "Bezahlt von", "An", "Geteilt", "Betrag", "Währung", "Umgerechnet (" + base + ")", "Ort", "Notiz"].join(";")];
      state.expenses.slice().reverse().forEach(function (e) {
        ...
        if (e.kind === "settlement") {
          ...
          rows.push([e.date, "Ausgabe", cell(catFor(e.category).label), cell(e.person || ""), "",
            e.shared === false ? "nein" : "ja", amt, e.currency, conv, cell(e.place || ""), cell(e.note || "")].join(";"));
        }
      });
      return "\uFEFF" + rows.join("\n");
    },
