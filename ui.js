...
(function () {
  ...
  function renderOverview(s) {
    ...
    if (s.trip.budget > 0 && ui.personFilter === "all") {
      var spentB = R.computeDerived("all").totalBase;
  ...
  function renderList(s) {
    ...
    var entries = s.expenses.filter(function (e) {
      ...
      if (!q) return true;
      var hay = [e.note, e.place, e.person, e.kind === "settlement" ? "ausgleich " + e.from + " " + e.to : R.catFor(e.category).label, e.currency, String(e.amount)]
        .join(" ").toLowerCase();
    ...
    if (!entries.length) {
      ...
      rows = groups.map(function (date) {
        ...
        var dayTotal = 0;
        dayEntries.forEach(function (e) { if (e.kind !== "settlement") dayTotal += R.convertExpense(e); });
        var wd = "";
        try { wd = weekdays[new Date(date + "T12:00:00").getDay()] + " "; } catch (err) {}
        var header = '<div class="day-head"><span>' + wd + escapeHtml(date) + "</span>" +
          '<span class="mono">' + R.fmtMoney(dayTotal, s.trip.baseCurrency) + "</span></div>";
        var items = dayEntries.map(function (e) {
          ...
          var dateLabel = e.checkIn
            ? escapeHtml(e.checkIn) + (e.checkOut ? " → " + escapeHtml(e.checkOut) : "") + (stayN ? " · " + stayN + (stayN === 1 ? " Nacht" : " Nächte") : "")
            : escapeHtml(e.date);
          var meta = dateLabel +
          ...
          return '<div class="expense-row" style="--cat:' + cat.color + '" data-action="edit-expense" data-id="' + escapeAttr(e.id) + '">' +
  ...
  function renderAddModal(s) {
    ...
    return modalShell(editing ? "Ausgabe bearbeiten" : "Neue Ausgabe",
      ...
      '</div><div class="hint">Einzelausgaben zählen zur Reise-Statistik, fließen aber nicht in den Ausgleich ein.</div></div>' +
  ...
  function renderSettingsModal(s) {
    ...
    return modalShell("Einstellungen",
      ...
      '<div class="hint" style="text-align:center;margin-top:2px">TripCash · Version 25</div>'
    );
  }
  ...
  function renderReport(s) {
    ...
    var days = groups.map(function (date) {
      ...
      var rows = byDate[date].map(function (e) {
        ...
        var cat = R.catFor(e.category);
        var info = [e.person, e.place, e.note].filter(Boolean).map(escapeHtml).join(" · ") + (e.shared === false ? " · privat" : "");
  ...
  function bindEvents(s) {
    ...
    function updateNightsHint() {
      ...
      hintEl.textContent = n > 0 ? (n + (n === 1 ? " Nacht" : " Nächte")) : "";
    }
    ["f-checkin", "f-checkout"].forEach(function (id) {
      var elD = document.getElementById(id);
      if (elD) elD.addEventListener("change", updateNightsHint);
    });
    updateNightsHint();
