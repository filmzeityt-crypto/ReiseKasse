...
(function () {
  ...
  // Bargeld-Abhebung: kein Ausgabe-Eintrag, sondern Geld, das in die gemeinsame
  // Bargeld-Kasse wandert. Wer abhebt, streckt das Geld vor.
  function addWithdrawal(exp) {
    var entry = {
      id: uid(), kind: "withdrawal",
      person: exp.person || state.sync.person || "Ich",
      amount: Number(exp.amount) || 0,
      currency: exp.currency || state.trip.baseCurrency,
      category: "sonstiges",
      date: exp.date || todayISO(),
      note: exp.note || "",
      place: exp.place || "",
      rateUsed: state.rates[exp.currency] || 1,
      baseAtEntry: state.trip.baseCurrency,
      shared: true, createdAt: Date.now()
    };
    state.expenses.unshift(entry);
  ...
  // Kassenstand je Währung: abgehoben minus bar bezahlt
  function cashBalances() {
    var map = {};
    state.expenses.forEach(function (e) {
      var cur = e.currency || state.trip.baseCurrency;
      if (e.kind === "withdrawal") {
        if (!map[cur]) map[cur] = { withdrawn: 0, spent: 0 };
        map[cur].withdrawn += Number(e.amount) || 0;
      } else if (e.cash && e.kind !== "settlement") {
        if (!map[cur]) map[cur] = { withdrawn: 0, spent: 0 };
        map[cur].spent += Number(e.amount) || 0;
      }
    });
    return Object.keys(map).map(function (c) {
      return {
        currency: c,
        withdrawn: map[c].withdrawn,
        spent: map[c].spent,
        rest: Math.round((map[c].withdrawn - map[c].spent) * 100) / 100
      };
    });
  }
  ...
  function computeSettlement() {
    ...
    // Wer hat wie viel Bargeld abgehoben (= in die gemeinsame Kasse gelegt)?
    var potBy = {};
    var potTotal = 0;
    state.expenses.forEach(function (e) {
      if (e.kind !== "withdrawal") return;
      var pw = e.person || "Ich";
      var vw = convertExpense(e);
      potBy[pw] = (potBy[pw] || 0) + vw;
      potTotal += vw;
    });
    var potNames = Object.keys(potBy);
    potNames.forEach(function (n) { if (!(n in paidBy)) paidBy[n] = 0; });

    // Barzahlung: nicht der Zahlende bekommt sie gutgeschrieben, sondern
    // anteilig die, die das Bargeld abgehoben haben (sonst doppelt gezählt).
    function creditPot(v) {
      potNames.forEach(function (n) {
        paidBy[n] = (paidBy[n] || 0) + v * (potBy[n] / potTotal);
      });
    }
    ...
    state.expenses.forEach(function (e) {
      ...
      if (e.kind === "withdrawal") return; // Abhebung ist keine Ausgabe
      if (e.reimbursed) return;            // wird von außen bezahlt
      var p = e.person || "Ich";
      var v = convertExpense(e);
      var fromPot = !!e.cash && potTotal > 0;
      if (e.shared === false) {
        // Einzelausgabe: neutral — außer sie wurde aus der gemeinsamen Kasse bezahlt
        if (fromPot) {
          paidBy[p] = (paidBy[p] || 0) - v; // hat Gemeinschaftsgeld für sich genutzt
          creditPot(v);
        }
        return;
      }
      total += v;
      if (fromPot) creditPot(v);
      else paidBy[p] = (paidBy[p] || 0) + v;
    });
  ...
  function computeDerived(personFilter, includeReimbursed) {
    ...
    var allEntries = state.expenses.filter(function (e) {
      if (e.kind === "settlement" || e.kind === "withdrawal") return false; // reine Geldflüsse, keine Ausgaben
  ...
  window.Reisekasse = {
    ...
    exportCsv: function () {
      ...
      var base = state.trip.baseCurrency;
      var rows = [["Datum", "Art", "Kategorie", "Bezahlt von", "An", "Geteilt", "Erstattet", "Bar", "Betrag", "Währung", "Umgerechnet (" + base + ")", "Ort", "Notiz"].join(";")];
      state.expenses.slice().reverse().forEach(function (e) {
        ...
        if (e.kind === "settlement") {
          ...
          rows.push([e.date, "Abhebung", "", cell(e.person || ""), "", "", "", "ja", amt, e.currency, conv, cell(e.place || ""), cell(e.note || "")].join(";"));
        } else {
          rows.push([e.date, "Ausgabe", cell(catFor(e.category).label), cell(e.person || ""), "",
            e.shared === false ? "nein" : "ja", e.reimbursed ? "ja" : "nein", e.cash ? "ja" : "nein",
